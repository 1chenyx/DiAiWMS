...
class ActionLogRepository(BaseRepository[ActionLog]):
    ...
    def __init__(self, db_session: AsyncSession):
        super().__init__(ActionLog, db_session)
    ...
    async def search_by_tenant(
        ...
        if search_params:
            if "action_type" in search_params:
                query = query.where(ActionLog.action_type == search_params["action_type"])
            if "user_name" in search_params:
                query = query.where(ActionLog.user_name.like(f"%{search_params['user_name']}%"))
        
        total_query = select(func.count()).select_from(query.subquery())
        total_result = await self._db_session.execute(total_query)
        total = total_result.scalar()
        
        query = query.order_by(ActionLog.create_time.desc())
