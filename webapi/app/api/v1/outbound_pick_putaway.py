...
from app.services.outbound_pick_putaway_service import OutboundPickPutawayService
...
async def create_outbound_pick_putaway(
    ...
    result = await service.get_by_id(pick_putaway_id)
    return success_response(result)
...
async def start_pick(
    ...
    """
    service = OutboundPickPutawayService(db)
    flag, msg = await service.start_pick(id, picker_id, picker)
...
async def complete_pick(
    ...
    """
    service = OutboundPickPutawayService(db)
    flag, msg = await service.complete_pick(id)
