...
class StockadjustService:
    ...
    async def update(self, data: StockadjustUpdate) -> Tuple[bool, str]:
        query = select(Stockadjust).where(Stockadjust.id == data.id)
        result = await self.db_session.execute(query)
        entity = result.scalar_one_or_none()
        ...
        entity.source_table_id = data.source_table_id
    ...
    async def delete(self, id: int) -> Tuple[bool, str]:
        query = select(Stockadjust).where(Stockadjust.id == id)
        result = await self.db_session.execute(query)
        entity = result.scalar_one_or_none()
