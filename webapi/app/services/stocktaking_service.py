...
class StocktakingService:
    ...
    async def update(self, data: StocktakingUpdate) -> Tuple[bool, str]:
        query = select(Stocktaking).where(Stocktaking.id == data.id)
        result = await self.db_session.execute(query)
        entity = result.scalar_one_or_none()
        ...
        entity.book_qty = data.book_qty
        entity.counted_qty = data.counted_qty
        entity.difference_qty = data.counted_qty - data.book_qty
    ...
    async def delete(self, id: int) -> Tuple[bool, str]:
        query = select(Stocktaking).where(Stocktaking.id == id)
        result = await self.db_session.execute(query)
        entity = result.scalar_one_or_none()
