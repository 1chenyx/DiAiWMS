...
from app.schemas.outbound.outbound_order import (
    OutboundOrderCreate,
    OutboundOrderUpdate,
    OutboundOrderViewModel,
    OutboundOrderItemViewModel
)
from app.core.current_user import CurrentUser
from app.repositories.outbound.outbound_order_repository import OutboundOrderRepository
from app.services.base_service import TenantAwareService


class OutboundOrderService(TenantAwareService[OutboundOrderRepository, OutboundOrder]):
    def __init__(self, db_session: AsyncSession):
        repository = OutboundOrderRepository(db_session)
    ...
    async def get_by_id(self, id: int) -> Optional[OutboundOrderViewModel]:
    ...
    async def _get_items_by_order_id(self, order_id: int) -> List[OutboundOrderItemViewModel]:
        query = select(OutboundOrderItem, Sku, Spu).join(Sku, OutboundOrderItem.sku_id == Sku.id)
        query = query.join(Spu, Sku.spu_id == Spu.id)
        query = query.where(OutboundOrderItem.order_id == order_id)
    ...
    async def create(self, data: OutboundOrderCreate, current_user: CurrentUser) -> Tuple[int, str]:
        query = select(Customer).where(Customer.id == data.customer_id)
        result = await self._db_session.execute(query)
        ...
        customer_name = data.customer_name if data.customer_name else customer.customer_name
        ...
        import uuid
        order_no = f"OUT{datetime.now().strftime('%Y%m%d%H%M%S')}{str(uuid.uuid4())[:8].upper()}"
    ...
    async def update(self, data: OutboundOrderUpdate) -> Tuple[bool, str]:
