...
class OutboundPickPutaway(WMSBaseModel):
    ...
    order_id = Column(Integer, ForeignKey('outbound_order.id'), nullable=False, comment='主出库订单ID')
    order_ids = Column(String(512), nullable=False, default='', comment='关联出库订单ID列表，逗号分隔')
    order_no = Column(String(64), nullable=False, default='', comment='出库订单号')
    order_nos = Column(String(512), nullable=False, default='', comment='关联出库订单号列表，逗号分隔')
