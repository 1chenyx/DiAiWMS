  ...
  <div class="stock-management">
    ...
    >
      <el-descriptions :column="2" border v-if="selectedStock">
        ...
        <el-descriptions-item label="创建时间" :span="2">{{ formatTimestamp(selectedStock.create_time) }}</el-descriptions-item>
        <el-descriptions-item label="更新时间" :span="2">{{ formatTimestamp(selectedStock.update_time) }}</el-descriptions-item>
      </el-descriptions>
      <template #footer>
...
const handleToggleFreeze = async (row: Stock) => {
  ...
  const confirmed = await confirmAction(`${action}这个库存`)
  if (!confirmed) return
  
  try {
    await stockService.update({ id: row.id, is_freeze: !row.is_freeze })
