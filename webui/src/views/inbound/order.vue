<template>
  <div class="inbound-order-management">
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span>入库订单管理</span>
          <el-button type="primary" @click="handleAdd">
            <el-icon><Plus /></el-icon> 添加入库订单
          </el-button>
        </div>
      </template>
    ...
    <el-dialog v-model="detailDialogVisible" title="入库订单详情" width="800px">
...
const dialogTitle = ref('添加入库订单')
const formRef = ref<FormInstance>()

interface OrderItemWithSku extends InboundOrderItem {
  sku_code?: string
  sku_name?: string
  spu_name?: string
}
...
const selectedOrder = ref<InboundOrderViewModel | null>(null)
...
const handleViewDetail = async (row: InboundOrderViewModel) => {
  try {
    const result = await inboundOrderService.getById(row.id)
...
const handleDelete = async (id: number) => {
  try {
    await ElMessageBox.confirm('确定要删除这个入库订单吗？', '警告', {
      confirmButtonText: '确定',
      cancelButtonText: '取消',
      type: 'warning'
    })
    
    await inboundOrderService.delete(id)
...
<style scoped>
.inbound-order-management { padding: 20px }
